...
test('R4 (AC-1, E9, E10, E11): the write hook keeps its guards — assertSignerMatches, publishOrThrow, injected hash8, target passthrough', () => {
  ...
  // disputeTag may be staging's one-liner or feat/tags' async block (contextual-pins story 3
  // refreshes the viewer's pins on dispute); what matters is the (tagInput, target) passthrough.
  assert(/const applyTag = useCallback\(async \(tagInput, target\)/.test(src) && /const disputeTag = useCallback\((?:async )?\(tagInput, target\)/.test(src) && /run\(tagInput, target, -1\)/.test(src),
