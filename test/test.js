...
const searchEvalHarness = require('./7-search-quality-eval-harness.test.js');
...
async function main() {
  ...
  const searchEvalHarnessResult = await searchEvalHarness.run();
  ...
  console.log(
    `search-quality-eval-harness suite:               ${searchEvalHarnessResult.fail === 0 ? 'PASS' : 'FAIL'} (${searchEvalHarnessResult.pass} passed, ${searchEvalHarnessResult.fail} failed)`
  );
  ...
  const overallOk =
    ...
    searchEvalHarnessResult.fail === 0;
