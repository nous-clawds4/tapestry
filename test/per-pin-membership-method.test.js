...
test('S2 (AC-5, E3-discipline): the dialog seeds from init, keeps the raw value, and spreads conditionally', () => {
  ...
  assert(!new RegExp(`membershipMethod:\\s*undefined`).test(inModule ? buildSrc : src),
    'the build must never emit `membershipMethod: undefined` — a conditional spread keeps a pre-story pin\'s blob byte-identical.');
