...
**Status:** Accepted (sub-decision 2, Amendment 1's wording and the candidates checkbox label superseded by `curated-dlist-update` ADR 0002; the page's single write by `curated-dlist-update` ADR 0003; AC-4 and sub-decision 8, the curation method placeholder, by `curated-dlist-update` ADR 0004; AC-5 and sub-decision 8, the Update list placeholder, by `curated-dlist-update` ADR 0005)
**Date:** 2026-09-11
...
> **Superseded in part (2026-09-13):** Update list is no longer a disabled placeholder (AC-5, sub-decision 8) — on my own lists it opens a preview of what my assistant would copy, refresh, delete and upgrade, built only from reads it could complete; nothing is signed yet — `curated-dlist-update` ADR 0005.
