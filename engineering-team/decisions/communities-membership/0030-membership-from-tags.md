...
6. **Production roster path + membership rule** → **RESOLVED + CORRECTED (Vinney's Claude, 2026-06-05 PM).** Consume the server: `GET /api/profile-tags/profiles-tagged?tagEventId=…&wotPov=house` → `aggregateProfilesTagged` (scan `#z=nostr-user-tag` + `#e=tagEventId`, dedupe replaceables, gate asserters by `wot_rank_<povSuffix> ≥ minRank`, **COUNT** apply/dispute per target — the system is **valence-naive**, it does NOT sum WoT weight). The **membership rule is `applyDisputesFunction`** (`trustedList/refreshPinnedTags.js:102`): **`applications ≥ cutoff AND applications > disputes`** — a two-part gate, **NOT** net-difference. *(This supersedes Option A's `score = Σ wot×polarity ≥ threshold` framing above — the design is count-based.)* Name mapping: our `membership_threshold` = server `cutoff` (min applications, honored); our `influence_cutoff` = server `minRank` (WoT gate) — but **`minRank` comes from `resolvePov` (house/user prefs), not the community**, so the per-community `influence_cutoff` field is **inert in v1** (forward-compat only). The app consumes the endpoint (app-as-consumer, gap #4); the server owns WoT/Meili. Our `lib/membership.js#deriveRoster` was **realigned to mirror the count + two-part gate exactly** — retained as semantic reference / test oracle / offline house-PoV fallback, not the production path.
7. **Writer + migration** → the carve is **read-only**, so the app publishes assertions (Story 43): tag-element (`z=39998:<LEG>:tag`, **bare-slug `d`**) + assertion (`p`, `e`, **`a=39999:<founder>:<slug>` from day one**, `polarity`, `d=profile-tag-<slug>-<target8>-<asserter8>`). Permissionless (asserter = signer; gate at read). Because we emit `a` from day one, **Communities needs zero backfill** when the server's `#a` read generalization lands. Interim: resolve a claimed `a` coord → latest tag-element event id and pass as `tagEventId`. Full wire sheet: `docs/COMMUNITIES_TAG_CORE_INTEGRATION_HANDOFF.md`.
