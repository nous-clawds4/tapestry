...
- **Known functional regression (accepted at implementation 2026-09-11; fix deferred to F6):** the legacy owner page `public/pages/nip85-control-panel.html` publishes a NIP-85 kind-10040 event by **reusing the login `session.challenge`** — `GET /api/get-kind10040-event` (returns `session.challenge`; `src/api/export/nip85/queries/kind10040.js:52`) then `POST /api/publish-kind10040-event` with an `X-Challenge` header (`.../commands/kind10040.js:101,121`). Making the login challenge **single-use** (an in-scope AC) ends that reuse, so this publish now returns 400 ("No active authentication session"). It was **already** broken after a React login (which deleted the challenge); post-fix it fails from every path. The CLI path (`bin/brainstorm-create-and-publish-kind10040.js`, via `SecureKeyStorage`) is unaffected. **F6 must give kind10040 its own challenge issuance, decoupled from login.** Operator decision (2026-09-11): accept and ship the login fix now rather than expand this story.
