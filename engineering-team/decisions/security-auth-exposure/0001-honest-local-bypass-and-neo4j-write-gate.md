...
- **Follow-ups:** `/api/firmware/install` is itself unauthenticated-callable (default-open) — belongs to the default-posture follow-up story.
- **Verified consequence of dropping `172.18.0.1` (confirmed at Review 2026-07-19):** a host → published `:7778` request arrives from the Docker bridge gateway (`172.18.0.1`), not loopback, so it is now treated as remote and requires auth. This is intended: on a droplet that same published `:7778` is internet-reachable and arrives from the same bridge IP, so trusting it was part of the exposure. The genuine local-operator path is **container loopback** (`docker exec <container> curl http://127.0.0.1:7778/…`) or the owner UI — both verified working unauthenticated for `localTrusted` writes. Local graph editing (e.g. the relationship-primitives tooling) uses those paths, not a naked host `:7778` curl.
