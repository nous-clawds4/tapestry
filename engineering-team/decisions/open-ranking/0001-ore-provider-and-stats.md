...
- `stats.js` — the **pure builder** `async buildStats({ pubkey, algorithm, pov }, deps)` returning a response triple `{ httpStatus, headers, body }` (`deps = { ownerPubkey, fetchProfileScores, isPovProvisioned }`): validate `pubkey` (`422` if missing/non-hex/`npub`); `resolveAlgorithm` (`422` if unsupported); if `algo.pov`: require+validate `pov` (`422` if missing/non-hex), `isPovProvisioned` (`422` + `X-Reason` if not — **never** fetch on a 422); if `!algo.pov`: ignore any `pov`; pick `observerPubkey` (`'owner'` for global / `pov === owner`, else `pov`); `fetchProfileScores`; map per the table; body `{ pubkey, rank, follows, followers, mutes, muters, reporters, ttl }`. Every triple's `headers` carry `Access-Control-Allow-Origin: *` + `Content-Type: application/json` (and `X-Reason` on errors). `handleStatsPubkey(req, res)` is the thin Express wrapper: build real deps (`fetchProfileScores`, `isPovProvisioned`, `ownerPubkey`), `await buildStats(req.body || {}, deps)`, then apply `httpStatus`/`headers`/`res.json(body)`.
- `capabilities.js` — also `buildCapabilityResponse()` → `{ httpStatus: 200, headers, body: buildCapabilityDocument() }` (headers = ACAO:* + json + `Cache-Control: public, max-age=300`); `handleCapabilityDoc` is its thin wrapper.
...
- Malformed-JSON 400: `oreJsonErrorHandler(err, req, res, next)` — a 4-arg Express error middleware (registered after the ORE routes) that, for ORE paths with `err.type === 'entity.parse.failed'`, responds `400` + `X-Reason` + `ACAO:*`; otherwise `next(err)`.
- **Testability seam (amended 2026-06-18 during Test Design; ratified by Story 1's test plan).** The decision logic lives in the **pure builders** `buildStats(input, deps)`, `buildCapabilityResponse()`, and `oreJsonErrorHandler` — each returning/writing a deterministic result from injected `deps` — and the Express handlers are thin wrappers. `index.js` re-exports `buildStats`, `buildCapabilityResponse`, `buildCapabilityDocument`, `oreJsonErrorHandler`, `isValidHexPubkey`, `registerOpenRankingRoutes`, `handleStatsPubkey`, `handleCapabilityDoc` so the suite drives behavior hermetically (no live Neo4j). Mirrors live-feed/0001's `buildFeed({deps})`. The Implementer **must** honor the injected deps — hard-wiring Neo4j into the handlers and skipping the builders breaks the (correct) hermetic tests.
