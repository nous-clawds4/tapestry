# ADR 0004: Architecture for the offline search-quality evaluation harness
...
Story #7 asks for an offline harness that scores search quality against a
...
- **Constraints.**
  - CLAUDE.md / `roles/architect.md`: JS-without-build; *no new
    lint/typecheck/build tooling without an ADR*. All four `.github/workflows/`
    are deploy-only — a PR-triggered CI workflow is net-new automation.
...
into an **ephemeral Meilisearch** (`MEILI_URL` override, v1.12.8 to match
`docker-compose.yml`) for each run. Hand-rolled binary-relevance scorer
(recall@k + MRR). Ships a PR-triggered CI workflow added as a **non-required**
check.
...
- **`test/7-search-quality-eval-harness.test.js`** — exports `async run()` →
...
- **`.github/workflows/search-eval.yml`** — PR trigger on `nostr-search/**` and
  `src/api/search/**`; Meilisearch **v1.12.8** service container; load corpus;
  `node nostr-search/eval/runner.js`. Added **non-required** in v1.
...
- Promoting the CI workflow to a required, merge-blocking gate (separate
  harness-contract ADR, for the user + Vinney to ratify).
