...
5. **Write the Build Audit** (`audit.md`). **Tick any §6 item another book, row or session has already resolved, and tick your own §6 items in the earlier audits that carry them.** The §6 registers are what `/whats-open` prints as closed-book carry-forwards, and nothing else ever ticks them: on 2026-09-20 they held 359 unticked items across 55 closed books, most of them resolved elsewhere long before. An unticked item that is done is not a harmless leftover — it is the reason the section stopped being read. The roll-up caps and dates what it shows (`WHATS_OPEN_CARRY_DAYS` / `WHATS_OPEN_CARRY_BOOKS`) so it stays readable, but only ticking keeps it *true*.
...
10. **Run the gate over the final state:** `npm test`; record the result in the audit — the run's `npm run gate:status` line ([Running and reading the test gate](../README.md#running-and-reading-the-test-gate)). The gate runs **after** the flip and the epic close-out so it certifies the tree the close actually leaves behind — in any other order L2 reports red on a correct close or green on an incomplete one.
...
13. **Close-out — after the gate and the close commit.** A book close is the only moment when the work is provably finished *and* still fresh; at the next session's start you would be reconstructing it, and in a dedicated cleanup session you would be doing archaeology. So the close carries its own tidy-up rather than leaving one for the operator to remember. Steps 1, 2 and 5 are **performed** — the first two because skipping them causes real problems, the last because it is provably lossless. Steps 3–4 are **judgment** and are *offered*, never done unasked. Classify every step: the first run of this workflow left step 5 unclassified, and it was performed anyway while two of the three places describing it said "offer" — an unclassified step does not stay undecided, it gets decided silently.
    ...
    2. **Push the branch the close landed on.** Not tidiness — **correctness on a multi-machine checkout.** Sibling sessions read `origin`, not this machine: ledger rows, stories and close artifacts that exist only locally are invisible to them, and work gets done twice or contradicted. (Ledger ids themselves no longer depend on it — a new row's id is a date and a slug, minted without looking at `origin`; ADR `ledger-row-identity/0001`.)
    ...
    5. **Fast-forward the local refs** for branches that only trail — **performed, not offered**: a ref strictly behind its upstream cannot lose work, the update never touches the working tree, and nobody else sees it, so an approval prompt would be friction bought with nothing. (A local `main` 30-odd commits behind `origin/main` after a run of promotions is normal and harmless — but it makes `git log main` quietly lie to the next person who reads it.) The distinction from step 4 is the point: branch *deletion* is irreversible and gets asked; a fast-forward is not and does not.
...
## Per-phase commit
Commit the audit, feedback doc, updated `book.md`, and any ledger rows (new files under `ledger/`; status flips in the `OPEN.md` table) together: `git add engineering-team/audits/<book-slug> OPEN.md ledger && git commit -m "book-close: <book-slug>"`. The step-13 close-out follows this commit.
