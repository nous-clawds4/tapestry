...
| AC-9 | A full ADR, Accepted, recording the seven named axes | `decisions/done/curated-dlist-update/0001-curation-copy-convention.md` — Status Accepted; Options 1 (link type, A–D), 2 (`q` / `e`-`a` / item-level `b` / named tag), 3 (deletion / republish / both), 4 (carries and drops), 5 (`d` derivation), 7 (one assistant vs one entry per (list, assistant)), 8 (keep / withdraw the facet) | Met |
...
- The story's `Status:` line is set to `Done` in place (`stories/done/curated-dlist-update/1-curation-copy-convention.md:3`);
