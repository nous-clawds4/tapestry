...
- [x] Files changed match the note's blast radius — `App.jsx`, `styles.css` (append-only, verified), `test/test.js`, plus the six new files. Extra: `.envrc` (N-2), `OPEN.md` row 292 (renumbered at the staging sync) and `protocols/worksheet.md` W17 (both harness/spec bookkeeping the note itself calls for).
