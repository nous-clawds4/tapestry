...
- **This is enforced by a single GLOBAL publish guard** (Story 2 / ADR 0002; operator decision 2026-06-26, *reversed* from the initial default-off sketch): the guard is **opt-in and local-only**. **Unset = publish externally as today** (production / staging / tags / other devs untouched, zero config change). Setting **`BRAINSTORM_PUBLISH_LOCAL_ONLY=true`** in a deployment's `brainstorm.conf` forces **local-relay-only across all publish paths** (incl. the existing `nostr-user-tag` path). The guard is enforced **client-side** at the `ui/src/utils/nostrPublish.js` chokepoint, **fail-open** (an unreadable policy falls back to external).
- **To honor THIS build's invariant you must set the flag.** It is **not** safe-by-default: on your local feat/tags dev box, set `BRAINSTORM_PUBLISH_LOCAL_ONLY=true` in `brainstorm.conf` and restart the container (set once, persists). The CJS test suite never runs the browser publish path, so tests cannot leak regardless. The Reviewer must still reject any diff that adds an external-publish path bypassing the guard.
