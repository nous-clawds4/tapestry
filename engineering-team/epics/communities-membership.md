...
- **45 — trust signal + applicant/member display** (Block 3: "N people you trust are inside" + per-member legibility + roles). 🟡 **Data layer DONE + reviewed (2026-06-05)** — `lib/roster.js` consumes the live `profiles-tagged` (ADR 0031). **UI remains** (People tab + trust signal) — the human-review point. v1 members-only (applicant role parked on a `selfApplied` endpoint flag).
