...
2. **search-pubkeys-global** — the **ORE-05 `/search/pubkeys`** endpoint, **global only** (`grapevine`, ranked by the instance's owner/global POV), backed by the Meilisearch profile index, **extending** the ORE-01 capability document to advertise it. *(this story — drafting now)*
3. **search-personalized** — adds `grapevine-personalized` to `/search/pubkeys`, gated on a server-side **main→delegated POV resolver** (owner→TA via config; customer→relay key via `getCustomerRelayKeys`) so ORE `pov` stays the human's main pubkey across both endpoints, with a Meili-column readiness check → `422` for unprovisioned POVs. Resolves the cross-store POV seam (worksheet **W13**). Also authors the BIBLE section for the as-built ORE implementation. *(planned)*
