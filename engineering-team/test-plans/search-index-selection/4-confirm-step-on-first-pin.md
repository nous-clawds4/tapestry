...
| **AC-5** "Pin to community" goes through the same interstitial | S2, S4, S10 | `handlePinToContext` no longer calls `pinTag` inline, carries the chosen context into the same dialog, and shares one publish; `PinToContextModal.jsx` unchanged vs HEAD | structure |
