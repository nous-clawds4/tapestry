...
- [x] **AC-5 (idempotent OUTCOME):** Reinstalling leaves exactly **one** `b` tag and **one** derived edge per concept — no duplicates (MERGE on the edge; one seed per run). _Clarified after live verification (ADR 0002 §"Live verification"): firmware re-publishes the TA headers every install (normal behaviour) and the emitter re-seeds the firmware-default `b`, so the header **is** re-published (new id) each install — but the outcome is idempotent. The never-clobber check protects **within** a run, not across a reinstall; an operator's manually re-pointed `b` reverts to the firmware default on reinstall (**accepted as defensible** — `OPEN.md` #8)._
