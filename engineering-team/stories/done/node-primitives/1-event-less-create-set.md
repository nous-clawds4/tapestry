...
- H10's fixture was amended in Phase 4 (`8b77d448`, an operator-approved kick-back to the Tester): a fresh instance has no `nostr-kind` elements, so H10 now brings two throwaway members.
- Round 2 (review finding B1 and non-blocking finding 2): the write learns whether it created the node from a marker set ON CREATE and removed in the same statement, instead of the ADR sketch's pre-MERGE `OPTIONAL MATCH`, so two concurrent identical calls can't both answer `created`. It writes the would-be tags only on create, and it refuses a node at the address that isn't the same-name Set (zero rows → 500), so an interloper is never adopted.
