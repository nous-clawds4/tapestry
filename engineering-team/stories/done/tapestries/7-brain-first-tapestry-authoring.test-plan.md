...
- Fixtures & residue (OPEN.md #128 lesson): live writes use **stable** d-tags — `tapestry-brainfirst-fixture-t7fixture` (TA-authored fixture tapestry; members drawn from firmware concepts `tag` / `nostr-user-tag`) and `test-brainfirst-thirdparty-t7` (**stable** throwaway-key letter; the key is a deliberately non-secret fixture literal). Each run **replaces** the prior run's addressable event: zero corpus growth, classifiable prefix for any future sweep. R2 additionally **self-cleans** its letter by exact event id on exit (best-effort `strfry delete`), so the permissionless View Tapestries directory carries no third-party row between runs. Residue per machine: exactly one fixture tapestry (relay + brain node). *Correction 2026-08-04 (Implementation phase, disclosed): the approved plan's first R2 used a per-run random key, which minted a new directory-visible coordinate every run — five junk letters accumulated during verification and were swept (`strfry delete`, dry-run-first, 5 events). The stable-key + self-cleanup shape above replaces it; assertion semantics unchanged.*
