...
- **Review CHANGES REQUESTED fix-up** (2026-06-08): the first review caught that the `/reporters` "About this data" popover still claimed a per-viewer/House model ("…the House (default) view is shown"), contradicting the new Owner PoV line (AC5). Fixed: popover ¶2 now reads "These counts reflect this instance owner's web of trust — a point of view. There is no single global number." (drops the per-viewer/House claims, keeps the true no-global-tally sentence so `verified-reporters-list-page` T8 stays valid). Added guard **T8** to the suite asserting NO `House (default)` anywhere in `BrainstormReporters.jsx` (the prior tests only checked the PoV *line*, so the popover slipped through).
