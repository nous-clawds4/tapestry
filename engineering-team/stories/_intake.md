...
**NOT PICKED UP.** Estate-wide tracking: [NosFabrica/protocols#6](https://github.com/NosFabrica/protocols/issues/6). Add [llms.txt](https://llmstxt.org/) — a root-level curated markdown map of key documents for visiting AI agents — to every tapestry-fleet host (four since `communities.` and `curate.` were decommissioned on 2026-09-12), as one more exact-match document through the site-trust plumbing this repo already ships.
...
| **S8** | **Phase and role boundaries** — **split 2026-09-20 into S8a–S8f, see below** | 22 (one row left the group) | Was 23 and too incoherent to build. The split moved row 16 to S3 and isolated one item that is not work at all |
...
5. **S8** is split (below); take **S8a** with S10 — both are lint-timing, both are hours. **S3**, **S4**, **S6**, **S9** and the one-offs after.
...
Read in full rather than sorted by their one-line leads, the 23 rows were five buildable groups, one
row that belonged elsewhere, and one item that is not work. Partition verified by script: every row
appears exactly once, none added, none dropped.

**Row 16 moved to S3, and is a likely verify-and-close.** Its L4-versus-doc-lane half was
dispositioned on 2026-08-18 — the non-numbered filename form is now the ratified convention, present
today in both `workflows/0-intake.md` §3 and `templates/review-checklist.md`, and `harness-lint`
emits INFO rather than a violation for the 13 non-numbered reviews in the tree. What is still open in
that row is unrelated to phase boundaries: a backfill decision for the `live-feed` epic's four
post-close reviews (#3–6, PASS verdicts, no story files), which is also what the four L4 waivers in
the session-start digest point at. That is epic lifecycle, so it belongs to S3.

| # | Story | Closes | Why these belong together |
|---|---|---|---|
| **S8a** | **The lint fires at the wrong moment in the phase cycle** | 2 — rows 98, 158 | Both rules key on committed state while the natural working order is to run the gate *before* committing, so the harness's own "commit at each phase boundary" convention is what creates the red. L10 is invisible until after the commit; L1 is red by construction between writing a PASS and receiving approval. Two rows, hours of work, and they make every other gate run trustworthy — **do this one first** |
| **S8b** | **Verification steps with no owner** | 4 — rows 160, 197, 206, 302 | Each is work that had an owner while the phases ran in a straight line and lost one the moment they did not: a hand-back strands the steps the phase had not reached (160), a gate never re-asserts itself after an interruption (197), a Phase-4 discovery has no lane back into Phase 3 (206), a test plan delegates a live check to nobody (302). In 160 the stranded step was the *only* guard that caught both blocking findings |
| **S8c** | **The record of what happened is lossy or false** | 6 — rows 80, 118, 309, 332, `2026-09-20-inplace-pass-erases-kickback-history`, `2026-09-20-human-gated-approvals-leave-no-record` | You cannot reconstruct a book from its artifacts. Non-blocking findings live only in the review file (80a); rewriting a verdict in place erases the only token the kick-back instrument greps; human-gated approvals are written down nowhere a Reviewer can check; the retro's instrument under-counts in-file rounds; nothing says when a PR may be opened relative to the Review, so an AC naming "the PR" is unmeetable. Row 118 is the sharp end — records that were *false*, every instance checkable in under a minute |
| **S8d** | **The harness's own artifacts contradict each other** | 5 — rows 212, 233, 298, 313, 316 | Wiring versus brief (316), template versus workflow (313), a Status value no workflow sets (233), an endorsed lane with no commit cadence (212), and instructions that can be newer than the branch under review (298). All are one-file edits once someone decides which side is right; the work is the deciding, not the editing |
| **S8e** | **An ADR or Design note is written without what it needs to be true** | 4 — rows 240, 243, 247, 311 | The Architect's conflict check cannot see a posture enforced by a sentinel rather than an ADR (243); a Design note omits the config it depends on (247); docs-mode has no consistency sweep, so restatements of a changed enumeration survive (240); a prescribed present-tense status is true only after a later story (311). All four are additions to the Architect's inputs checklist |
| **S8f** | **Not work — a decision owed by the operator** | 1 — row 343 | *May a Reviewer search a session's transcript?* Reviews label a session's own observations "cannot be re-observed" while that session's transcript sits on the same disk, and one review has already read one. This wants a ruling, not a story, and it should not sit in a story queue pretending to be buildable |
