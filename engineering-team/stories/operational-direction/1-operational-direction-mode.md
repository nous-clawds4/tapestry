...
- Review: `engineering-team/reviews/operational-direction/1-operational-direction-mode.md` — **CHANGES_REQUESTED** (2026-07-26). AC1/2/3/5 verified; **AC4 only partially** — the boundary invariant is enforced only when a caller injects a verdict, and the endpoint never does, so `eligible: true` can be returned over unjudged steps once the anchor-distance policy parameter is raised. Route: Architect (ratify fail-closed shape + amend ADR d6) → Tester (unjudged-steps coverage) → Implementer (make the core refuse).
