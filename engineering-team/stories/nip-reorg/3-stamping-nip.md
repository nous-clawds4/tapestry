...
- [ ] **AC2 — the ratified write rule, D2 vocabulary.** The spec states the full convention: personal `z` required (≥1, may target a private header — the local-first premise); up to a cap of cloud handles, **affiliation-anchored** (the author's declared community via their own pointer-`b`, never a concept-global top-k); `z` order not load-bearing; re-stamping by lazy author re-emit with the named lossiness (foreign-authored, inactive-author, kind-9999 items); the containment-only boundary (membership assertions keep their single applied-concept handle). Framed as consuming `b` (Shared Concepts / Inherit-From) and `s`/`n` (Class Thread Relationships, by its new name). Amended 2026-07-12: an explicit sentence conditions the convention on the author's publish intent (privacy).
- [ ] **AC3 — the read-side contract, same page.** An explicit consumer-facing section: what a reader MAY assume (personal `z` present), what it MUST NOT rely on (`z` order; cloud stamps being current; ancestor stamps existing — pending O1), and what that implies for query strategy. Amended 2026-07-12: readers MAY infer omitted stamps by walking `s` structure and the `b` graph (subject to the class-thread security gates, from their own observer position) and MUST NOT assume other consumers perform inference.
- [ ] **AC4 — O1 opened properly (amended 2026-07-12, user-gated).** An "Open: which layers to stamp (set × branch)" section frames the valid-`z` space along two axes — set layers via `s` (fine/coarse-grained, dynamic ladder) and branch layers via the author's `b` graph (proximal/distal, **indirect linkage valid**, reach affiliation-backed, correspondence-closure semantics flagged as unspecified) — and presents the candidate selection principles neutrally: anticipated filter demand (either axis), proximal+distal endpoints (noting convergence with the ratified rule), and read-time inference as the capability-dependent complement. The stakes stated as the interop floor for non-expanding clients (smart clients recover omissions; dumb ones don't); the co-stated-read-contract rule retained; the Widgets example retained. No leaning — settling remains a future `/discuss`. Worksheet **W14** mirrors the framing (heading/anchor unchanged), cross-linked both ways.
