...
|---|---|---|---|---|
| AC-1 | Inline TAGS section visible on profile with Manage link | `profile page renders an inline TAGS section with a Manage link` | `tests/brainstorm/profile-tags.spec.js` | Playwright |
| AC-2 | Add affordance + AddTagDialog typeahead + create-new flow | `TAGS section exposes an add affordance (empty-state or +)` + `clicking the add affordance opens an Add tag dialog with a search input` + `GET /api/profile-tags/available-tags returns { success, tags: [] }` + `available-tags lists a kind-39999 tag-element after it is published` | spec.js + `test/profile-tags.test.js` + `test/profile-tags-publish.test.js` | Playwright + API contract + live publish |
...
| AC-4 | Dispute publishes assertion with explicit negative polarity | `dispute (polarity=-1) appears under disputes, not applications` | `test/profile-tags-publish.test.js` | live publish |
...
| AC-6 | Profile shows WoT counts and asserters per tag | API round-trip asserts `entry.tagEventId` + `entry.authorPubkey` so the chip popover can render counts and asserter rows (verified by `apply (polarity=1) appears under applications` + `dispute (polarity=-1) appears under disputes`) | `test/profile-tags-publish.test.js` | live publish |
| AC-7 | Manage view exposes list + revoke | `Manage button opens a Manage dialog` + `publishing a kind-5 deletion removes the asserted entry from the API response` | spec.js + `test/profile-tags-publish.test.js` | Playwright + live publish |
...
| AC-11 | POV-WoT-tagged profile appears in search with matched-tag chip; assertions from anyone in the POV's WoT count, not just the searcher's own | `typeahead search returns a profile tagged by a third-party author, with _matchedTags on the hit` | `test/profile-tags-publish.test.js` | live publish + meili proxy integration |
