...
- [ ] **F4 — publish-time default stamping**: implement the ratified stamping floor (personal `z` + the joined shared concept's handles) in the authoring flows; gap = the resolver + the remaining single-z writers. **In flight (staged)** — story: `stories/shared-concepts-adoption/4-publish-time-default-stamping.md`. `/discuss` decisions 2026-08-06: **the choice IS the pointer-b** (the intake's registry-field sketch deferred to a multi-affiliation-ordering future); stage 1 = pin/TL personal-z parity + the resolver + the create-element seam; the client-built-writer sweep is a named follow-up (candidate list produced by this story's Architect).
