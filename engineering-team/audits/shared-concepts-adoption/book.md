...
- [ ] **F2 — inverse queue (self-declare candidates)**: my headers with cross-author z/b usage but no self-pointing b → prompt "Submit as a Shared Concept" (the existing button is the action). **In flight** — story: `stories/shared-concepts-adoption/3-inverse-queue-publish-candidates.md` (second view on the Adoption Queue page; decline = the F5 sentinel — no new machinery; deferred-but-in-use behind a collapsed reveal — owner decisions 2026-08-06).
