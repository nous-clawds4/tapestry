...
- [ ] **F1 — adoption-candidates queue**: S3 (∪ S3a/S3b) ∖ S2a → a review surface prompting the owner per concept (adopt via pointer-b on the local twin header and/or create the registry record). Proposal-loop shape: the system nominates, the owner ratifies. *The generalized b-append endpoint it consumes originates in F5 (story `shared-concepts-adoption` #1) — owner decision 2026-08-06, inverting the intake's F1→F5 order.*
...
- [ ] **F5 — concept-header b-coverage audit + guided disposition**: coverage tracking on Concept Headers plus the owner-prompted disposition flow (wire to external / self-declare / deliberately private). **In flight — leads the suite by owner priority (2026-08-06).** W16 ruled: the sentinel `["b", "b-tag-deferred"]` (owner decision at `/discuss`, 2026-08-06); the ruling + spec edits ride the story. Story: `stories/shared-concepts-adoption/1-b-coverage-audit-and-disposition.md`. Carries the wire-external b-append primitive F1 consumes.
