...
- [ ] **F1 — adoption-candidates queue**: S3 (∪ S3a/S3b) ∖ S2a → a review surface prompting the owner per concept (adopt via pointer-b on the local twin header and/or create the registry record). Proposal-loop shape: the system nominates, the owner ratifies. *The generalized b-append endpoint it consumes originates in F5 (story `shared-concepts-adoption` #1) — owner decision 2026-08-06, inverting the intake's F1→F5 order.* **In flight** — story: `stories/shared-concepts-adoption/2-adoption-candidates-queue.md` (dedicated queue page; dated supersedable decline record; S3b deferred to F3 — owner decisions 2026-08-06).
