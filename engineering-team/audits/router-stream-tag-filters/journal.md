...
## 2026-07-16T13:55:32Z — Promoted to production: PR #362 merged, deploy green, smoke clean
**Story/Phase:** relay-management #2 / prod promotion (operator-authorized)
**Decision:** INFO
**Judge:** n/a (operator explicitly authorized: "Let's go ahead and cycle to production" — the reserved-decision gate this book held for the operator)
**Why:** Bundle main..staging = PR #361 (this book) + docs-only #358 (OPEN.md #30 bookkeeping) — verified before opening PR #362. Merged 2026-07-16T13:52:48Z (merge 3a4b653e); required stack-free check green (28s); deploy-tapestry run 29504095698 succeeded (~86s). tapestry.brainstorm.world smoke: Tier 1 stable 3×2s; Tier 2 all 200 + search hits; Tier 3 — prod bundle index-CUstfBx5.js **byte-identical (cmp) to the staging-verified bundle**, stream-editor hint present, 'Tag Filters' ×2; /api/strfry/router-status: router RUNNING post-restart with prod's own 7 streams intact (per-instance state as designed — prod carries dcoslUpload where staging has treasureMaps; dcosl filter byte-unchanged) → the sanitizer path loads clean against prod's real config; Tier 4 covered by staging's console-clean pass on the byte-identical bundle (passive-prod preference honored — no prod browser drive, no mutations); Tier 5 — count endpoint echoes #z with correct local discrimination and the documented dcosl NIP-45 note. Book bullets now satisfied through prod; tags-branch decision still the operator's.
**Next:** Offer book close (operator ratifies) + tags-branch decision.

## 2026-07-16T14:10:27Z — Merged to feat/tags (operator-authorized): PR #363
**Story/Phase:** relay-management #2 / tags-branch decision (operator-reserved, now executed)
**Decision:** INFO
**Judge:** n/a (operator: "cherry pick the Router Management feature from staging and merge it into the tags branch")
**Why:** Executed as a staging→feat/tags sync merge (the #359 precedent) rather than a literal cherry-pick — the staging delta was exactly this feature's 11 commits, so the merge is the requested cherry-pick with clean ancestry. PR #363 (branch merge/staging-into-feat-tags-2) merged 2026-07-16T14:09:56Z, merge 2311487f. One conflict: OPEN.md numbering (tags line had renumbered into #31–37) — staging's #31 renumbered to #38 per the tags-line convention. Post-merge all five feature files byte-identical to the prod-verified versions (tags side had no genuine edits). Verification on the merged tree: full stack-free npm test Overall PASS exit 0 (tags battery + sibling 20/20 + story 21/21); vite build clean 15.22s with the tags branch's real deps (worktree-local npm ci — the tags line adds react-virtuoso, absent from the staging-era symlinked node_modules that first showed a false build failure). Coexistence noted: vcavallo's open #360 goes the opposite direction (feat/tags→staging) from an older base; their next re-merge absorbs this normally. Surfaced gap: deploy-tags.yml still triggers on feat/pubkey-tagging-target (pre-rename), so feat/tags merges do NOT auto-deploy tags.brainstorm.world — the running instance is unchanged until a manual deploy or workflow fix (OPEN.md #14 drift family).
**Next:** Report; book-close offer stands (all reserved decisions now executed: prod ✓, tags ✓).

## 2026-07-16T14:24:51Z — CORRECTION: feat/tags DOES auto-deploy; tags.brainstorm.world redeployed and smoked clean
**Story/Phase:** relay-management #2 / tags deploy (correction to the #363 entry)
**Decision:** INFO
**Judge:** n/a
**Why:** The prior entry's "deploy-tags.yml still triggers on feat/pubkey-tagging-target, so feat/tags merges do NOT auto-deploy" was WRONG — that is only STAGING's stale copy of the workflow; push-event triggers use the pushed branch's copy, and feat/tags' own copy was repointed to feat/tags on 2026-06-19 (85683b46, "ci: repoint deploy-tags.yml…"). Deploys have fired off feat/tags ever since (successful runs 07-07 through 07-16), including run 2026-07-16T14:10:02Z (success) triggered by the #363 merge. Read-only smoke of tags.brainstorm.world post-deploy: stable 3×2s; new bundle index-BLjSXI9c.js carries the stream-editor hint (1) + 'Tag Filters' ×2 → the router feature is LIVE on the tags instance; router running with its per-instance 6 streams intact; tags-line surface healthy (/api/profile-tags/available-tags 200 with data — an earlier 404 was a mis-guessed path, not breakage). Net state: the feature is live on all three instances (staging, prod, tags). Residual real item: staging's copy of deploy-tags.yml still names the pre-rename branch — one-line repoint to mirror feat/tags' copy, OPEN.md #14 drift family.
**Next:** Surface correction to operator (done in-session); offer the staging-copy workflow fix + a PR #363 comment correcting its deploy note.

## 2026-07-16T14:33:17Z — Two operator-authorized cleanups executed (workflow repoint + #363 correction comment)
**Story/Phase:** relay-management #2 / post-ship cleanups
**Decision:** INFO
