...
- **Operator validation on staging (2026-08-05, post-close):** the operator created a new tapestry on staging and saw it in **both** View Tapestries and the tapestry concept's Elements view (`39998:8e901369…:tapestry`), *"all of which is as intended."* They additionally exercised a path the suite could not: **editing a pre-#7 tapestry (add-a-concept) brain-wrote it** — previously absent from the Elements view, present in both views after the edit. This confirms an emergent as-built property: the hook fires on every *owned republish*, so any legacy tapestry the owner touches self-backfills (`importEventDirect` MERGEs the node fresh). Wholesale backfill of **untouched** legacy elements remains stage 2 (#136).
...
- Test gate at close (run over the final tree, after the epic close-out and waiver removal) — **honest record, corrected post-commit:** the first close-gate run was **Overall: FAIL** — a single failure, `relationship-primitives` H8, whose count-bracket assertion caught one ambient inbound event on the live-router relay (`6599652 → 6599653`; its own failure text names the concurrent-publisher class; this book's fixtures ruled out — the R2 letter count was 0). This book's suite was `PASS (19/0/0)` and harness-lint clean **without** the L2 waiver inside that same run. H8 re-ran green immediately (`relationship-primitives 23/0/0`), and the **full re-run over the identical tree was `Overall: PASS` (exit 0)** (logs: session scratchpad `close-npm-test.log` / `close-npm-test-2.log`). The close commit initially cited PASS on a mis-verified chain — see §7 for the disposition of both lessons (the flaky bracket, and the verification error).
...
| `relationship-primitives` H8's count-bracket assertion is flaky-by-construction on a live-router relay: it asserts *global* scan-count equality around its cycle, so any ambient inbound event fails it (observed at this close's gate, `6599652 → 6599653`; re-run 23/23; unrelated to this book's diff — fixtures ruled out by direct count) | close-gate run 1 (`close-npm-test.log:2363`) | **OPEN.md `meta` row #141** (candidate fix: scope the bracket to a filter the test owns, or one auto-retry before failing; test-hermeticity-ci territory) |
| The first close commit cited the gate as PASS on a mis-verified chain: the background wrapper's `; echo EXIT:$?` masked npm's real exit code, and the commit chain gated on grep *finding* the `Overall:` line rather than on its content — caught immediately post-commit and corrected by the §5 amendment in the follow-up commit | close session (this audit's own history) | **Declined** as a harness change (single-session verification-discipline error, self-caught by reading the evidence; the standing record-the-actual-result rule is what surfaced it). Portable note carried in row #141's text: gate on content (`grep -q 'Overall: *PASS'`) and never chain `; echo $?` after a gate command |
