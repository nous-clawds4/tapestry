...
- **Test gate — CI (binding):** `stack-free` **PASSED** on PR #411 (the staging→main promotion carrying this book). Per OPEN.md #27, CI is the binding gate for this repo because the local suite is environmentally unreliable.
- **Test gate — local `npm test`: INCONCLUSIVE (did not complete).** Started at close and still running after **~25 minutes** with no output. Not treated as a pass and not treated as a failure — it produced no verdict. This is consistent with, and further evidence for, the known local pathology: OPEN.md #27 (the local suite fails environmentally) and #51 (stream-consumer ETL saturates the Meili task queue, 5–8 min per single-doc task), compounded by #69 — root-caused *this session* — where the local container image predates the `stream-consumer` ETL entirely, leaving Neo4j with **3** `NostrUser` nodes against strfry's **3,067,260** events. **The book's changes touch none of those paths**: zero backend, zero data, zero POV logic, +177/−1 confined to `ui/`. Recording the non-result rather than quietly omitting it or waiting indefinitely; the binding gate above is what this close relies on.
- **Browser verification (the actual gate for this book):** all four routes render with no console errors on **local**, **staging**, **prod**, and **tags**. Deployed bundle hash matched the locally verified artifact on every hop (`index-6kIYzOex.js` → `index-CFJQP-zt.js`).
