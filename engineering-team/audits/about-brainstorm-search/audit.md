...
> **Landing note (2026-09-18).** This audit was written on 2026-07-22 on `ops/open72-73-ci-gate-and-tags-safety-gap`, a branch that was never merged; it reached the shared line on 2026-09-18 (PR #671). The body below is as written, with one mechanical change at the landing: four of the ledger rows it files collided with numbers `staging` had used in the meantime, so their citations were repointed — **74→317, 75→318, 76→319, 77→320**. Rows 72 and 73 kept their numbers; `staging` had reserved them for this branch. What became of the rows it cites: **73** resolved itself the day it was filed (`feat/tags` took the endpoint through `30e4ff68`); **317** duplicates row 29 and **320** duplicates row 43, so both landed closed; **72**, **318** and **319** are still open. One claim in §5 did not survive checking and is left as written: `418049a1` did not introduce the gate defect — it appended its term above a terminator that was already there, which row 43 had recorded on 2026-07-16. The defect was fixed under row 43 on 2026-07-25, and the hand-written chain has since been replaced by `test/registry.js`. Separately, `book.md` said the epic was retired "to `epics/done/`"; §7 below records that move as reverted, and the manifest now says so. A second mechanical change followed the same day: §1's story path was repointed when the epic's story folder moved under `stories/done/` — the close-out step the book close itself never made.
...
- **A visitor-facing front door explaining what Brainstorm Search is and the three ways to use it** — `stories/done/about-brainstorm-search/1-about-page-and-agentic-placeholder.md`
...
| **`harness-lint` L2 silently under-reported.** Its epic-slug parser matches ``- `slug` `` but not ``- **`slug`** ``. This book's manifest used the emphasised form on first draft, so lint ran green having evaluated **zero** epics for the book. Reformatting surfaced two real hits immediately. Green-and-wrong is the worst failure mode for a checker. | Discovered during this close, reconciling epic retirement | **OPEN.md row 319** — loosen the extractor to tolerate emphasis; consider flagging a Closed book that yields zero parsed epics |
