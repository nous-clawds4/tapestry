...
- [ ] Automated tests cover, at minimum: add-new, add-idempotent, delete-existing, delete-missing, nonexistent endpoint node, and rejected `relType` — plus the no-strfry-write assertion above.
...
1. Route naming and mount point (e.g. under `/api/normalize/*` to inherit its auth and conventions, vs. a new namespace).
2. Initial `relType` whitelist membership — `HAS_ELEMENT` + `IS_A_SUPERSET_OF` only, or the core-node wiring types as well.
