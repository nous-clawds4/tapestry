...
**Confidence:** high for the as-built (every story reviewed PASS by an independent reviewer, six of seven with a live reproduction on the local stack); **medium for "works end to end with a real signer"** — no story could sign a Map or publish a header under a real session, so the add → sign → publish chain had been exercised only up to the signer, never through it — **raised to high post-close (2026-09-10, after PR #622):** the operator's first real Add on staging is verified from relay data (§5 "Post-close verification")
...
- **Post-close verification (2026-09-10 evening, staging, real NIP-07 signer).** The operator added curation of `dog-breed`. Read back from the relays: the owner's Map (`e5272de9…`, `created_at` 2026-09-11T00:36:16Z) carries `["39998:dog-breed", <assistant 8e901369…>, "wss://dcosl.brainstorm.world"]` alongside its `30392` entry and `30382:*` rows (nothing clobbered); the assistant's kind-39998 `dog-breed` header is on `wss://dcosl.brainstorm.world` with the names copied ("dog breed") and `["b", "39998:11f23fe4…:dog-breed", "inherit-items"]` pointing at the self-declared community header; two other `dog-breed` headers on that relay carry no `b` and were correctly not offered. The operator sees the updated Map on both staging and production. The "never through the signer" caveat above is closed. **Second user, same instance (01:46Z):** a different signed-in account (`0f6c8526…`) added `dog-breed` too — its Map names a *different* assistant key (`253d40c4…`, not the owner's `8e901369…`) and that assistant's header is on the community relay with the same `inherit-items` pointer. Per-user assistant keys hold on a shared instance (OPEN.md row 188's invariant), and two users' curations of one community list coexist as two headers.
