...
| 4 | cleanup | The `live-feed` book's **feature** is merged to `staging` (PR #296, `80a39afd`), but the **post-merge audit trail** (Stage 2–3 journal entries + `completion-report.md` + `tier4-dom-extract.txt` + `audit.md` + `prd-seed.md` + the book-close commit) lives only on the local `feat/live-feed` branch. Merge it to `staging` as a **docs-only PR**, then delete `feat/live-feed` (local + remote). | 2026-06-15 (live-feed) | OPEN | — | Branch `feat/live-feed`; audit dir `engineering-team/audits/live-feed/`. Engineering hygiene + deferred scope from this book live in `audits/live-feed/audit.md` §6 (no need to duplicate here). |
