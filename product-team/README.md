...
- `.claude/agents/<role>.md` — subagents with role-appropriate tool whitelists. Writing roles carry permission rules scoping Write/Edit to `product-team/**` + root `OPEN.md` + `ledger/**` (out-of-tree writes require approval under default permission modes); the Product Advisor has no Write and no Bash.
