...
- A spawn prompt that gives a line-range command for a partial read (e.g. `sed -n '1,36p' <file>`) is **exact and exclusive**: run that command, use its output, and do not open the file any other way. Over-reading past the instructed scope breaks your blinding even when self-inflicted — a Gate-5 APPROVE was voided exactly this way. *(Ratified 2026-08-04, OPEN.md #133.)*
