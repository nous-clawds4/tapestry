...
**House rules:**
- Concept Graph: orient via `http://localhost:$TAPESTRY_PORT/api/concept-graph/summaries` per [AGENTS.md](AGENTS.md) before reading source. If the probe fails (stack absent), follow the AGENTS.md §2 fallback ladder (`firmware/*.json` → BIBLE ToC) and flag any concept handles you cite for re-verification against a live graph.
