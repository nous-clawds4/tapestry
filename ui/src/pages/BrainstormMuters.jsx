...
import useGrapevineMuters from '../hooks/useGrapevineMuters';
...
const PAGE_SIZE = 50;
// Muters-specific localStorage key — distinct from the follows/followers/reporters page
// keys so the pages do not clobber each other's column-visibility prefs.
const STORAGE_KEY = 'bsp-muters-columns';
...
export default function BrainstormMuters() {
  const { pubkey } = useParams();
  const navigate = useNavigate();
  const { user, login, logout } = useAuth();

  const { data: muters, loading, error } = useGrapevineMuters(pubkey);
  ...
  // Batch-load names/pics for the muter pubkeys. /api/profiles caps at 50 pubkeys/request,
  ...
  useEffect(() => {
    if (!muters || muters.length === 0) { setProfiles({}); return; }
    let cancelled = false;
    setProfiles({});
    const pubkeys = muters.map(f => f.pubkey);
    (async () => {
    ...
    <div className="bsp-page">
      ...
      <div className="bsp-content">
        ...
        <div className="bsp-follows-header">
          ...
          <h1 className="bsp-follows-title">Verified Muters</h1>
          <button
            type="button"
        ...
        {/* Body */}
        {loading && <div className="bsp-loading">Loading muters…</div>}
        {error && !loading && (
          <div className="bsp-trust-unavailable"><span className="bsp-trust-icon">🔒</span><span>{error}</span></div>
        )}
        {!loading && !error && rows.length === 0 && (
          <div className="bsp-empty">No verified muters found for this account.</div>
        )}
        {!loading && !error && rows.length > 0 && (
          <DataTable
            columns={columns}
