...
**Status:** Open · raised 2026-07-12

When an item joins a concept that has superset ancestors (`s` tags — [class-thread-relationships spec](./drafts/class-thread-relationships.md)), does the published item carry `z` stamps for the ancestors as well as the joined concept? The [stamping spec](./drafts/stamping.md) § "Open: subset/ancestor stamping" holds the running example (Widgets / Widgets for Carpenters / Widgets for Electricians) and the candidate shapes: **(a)** read-time `s`-walk expansion (stamp the joined concept only; readers union `#z` per subset), **(b)** write-time ancestor stamping (single-filter breadth queries, at the cost of denormalizing hierarchy into signed history — stale on re-parent, lazy-heal-only, ~2 `z` slots per chain level), **(c)** hybrids (e.g. direct + root only). Binding constraint from the stamping read contract: whichever shape lands MUST co-state what readers may assume (ancestor stamps present, or expand-at-read). Successor question to graduated [W11](#w11--cloud-formation--multi-z-stamping-rules), carved out at `nip-reorg` S3.

**Refs:** [stamping spec](./drafts/stamping.md) § "Open: subset/ancestor stamping"; [class-thread-relationships spec](./drafts/class-thread-relationships.md); `docs/NIP_REORG_DESIGN_HANDOFF.md` O1; `nip-reorg` ADR 0003.
