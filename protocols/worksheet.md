...
## W17 — Absent `polarity` on handshake-style Taggings

**Status:** Open · raised 2026-09-05

[tags.md § Polarity](./drafts/tags.md) reads an absent `polarity` tag as apply. The Brainstorm Cafe's Sponsor–Agent pairing, a *two-way handshake* of Taggings, counts a Tagging as a live claim only when `polarity` is explicitly `"1"`, and reads absence (like `"-1"`, deletion, or an unresolvable event) as a revocation, on the grounds that a handshake should be unambiguous. That is stricter than the draft. The question is whether the draft should define a class of relationship or handshake tags for which absence is *not* apply, or whether this is per-consumer strictness that any reader may adopt without the draft saying so. Publishers written for the Cafe always emit `polarity` explicitly, so nothing breaks either way today.

**Refs:** [tags.md](./drafts/tags.md) § Polarity and W3 (valence arc); the Cafe's pairing spec ([trusted-agents `docs/PAIRING.md` § 3](https://github.com/nous-clawds4/trusted-agents/blob/main/docs/PAIRING.md)).
