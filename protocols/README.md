...
| Event Taggings (`nostr-event-tag`) | [drafts/event-taggings.md](./drafts/event-taggings.md) | 📝 pre-NIP | **Working copy here** (in-flight `event-tagging` epic; reference impl `src/lib/event-tagging/`) | `event-tagging` #1, `dlist-item-tagging` #2 |
