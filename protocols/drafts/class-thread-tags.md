...
**Candidate letters.** Further relationships (`IS_A_PROPERTY_OF`, additional editorial types) are candidates for future single-char assignments — `REFERENCES` no longer is: it rides the `b` tag's `"pointer"` type ([Inherit-From & Resolved Definition](./inherit-from.md), per `community-reference` ADR 0029), which resolved worksheet [W5](../worksheet.md#w5--references-publishing-semantics). The cross-spec letter registry is an open protocol problem tracked as worksheet [W2](../worksheet.md#w2--single-char-tag-namespace-registry). This spec assigns no new letters.
