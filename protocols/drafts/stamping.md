...
**Curated copies.** A copy an assistant files under a curated list ([Assistant Designation](./assistant-designation.md) § "Curation copies") carries only that list's `z`: it is never stamped with the handles of the list its original came from, which the original already occupies.
