...
**Multi-`z` stamping (Tapestry-layer convention — `community-reference` ADRs 0029, 0033).** A deliberately-published item MAY carry multiple `z` tags — its personal parent pointer plus stamps naming the shared concepts it joins (the base NIP permits multi-`z` items while recommending one `z` per event as default practice). The convention — the write rule, re-stamping, the read contract, and the open subset/ancestor question — is **normative in [Stamping](./stamping.md)**; the cloud model those stamps draw on is specified in **[Shared Concepts](./shared-concepts.md) § Clouds**. Design provenance: worksheet [W11](../worksheet.md#w11--cloud-formation--multi-z-stamping-rules) (graduated) and `community-reference` ADR 0033; extracted from this section per `nip-reorg` ADR 0003.
