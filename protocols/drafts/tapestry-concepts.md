...
**Multi-`z` stamping (Tapestry-layer convention — `community-reference` ADRs 0029, 0033).** A deliberately-published item MAY carry multiple `z` tags: its personal parent pointer plus stamps naming the shared/community concepts it joins. The base NIP explicitly permits multi-`z` items while *recommending* one `z` per event as practice; Tapestry takes the multi-stamp convention for items published for community visibility, because read-side derivation cannot reach items whose personal headers are unpublished (local-first publication: most personal headers never leave their local relay, so a published item must be self-contained to be discoverable).

*The following is design (ADR 0033), framed as the convention — not yet wired: it reads the W1 grapevine-resolved consensus signal, which (like the resolver) does not exist on this deployment. Implementation is gated on on-wire `b`-tags and the three-branch reconciliation. The exact cap, ranking formula, and firmware cold-start cluster contents are deliberately deferred to that implementation.*

- **The cloud.** The "cloud" of headers for a concept is the **derived top-k of the W1 grapevine-resolved consensus signal** (a concept's incoming inherit-typed `b` edges, weighted by each child author's GrapeRank influence from the observer's PoV — see [Inherit-From](./inherit-from.md) § Aggregation). It is **never a published object or manifest**: there is no signed "cloud" event and no curator (a manifest would reintroduce a privileged center). Membership is consensus rank; mutual pointer-typed `b` edges are the author's **navigation** to the cloud (follow your header's `b` to the community it points at), **not** a membership gate.
- **Rotation is emergent.** Nobody governs cloud membership; it changes as the signal changes, and there is nothing to "detect" — an author (at write time) and a consumer (at read time) each simply recompute. The selector follows the same precedence as the community-reference model (BIBLE §22): `grapevine-resolved top-k → firmware-blessed cluster (cold-start) → none`. An organic (non-firmware) concept's cloud **bootstraps from a singleton** (the founder's header) and thickens as deference accumulates.
- **Stamping is affiliation-anchored.** An author stamps the cluster of the community they **declared** affiliation with (reached via their own pointer-typed `b`), not the concept's global top-k — so an item carries the author's *declared* associations and is not silently routed into a community they never chose.
- **The stamp.** A deliberately-published item carries its **personal `z`** (required — at least one parent pointer per the base NIP / BIBLE §10 Rule 2; it MAY point at a header the author keeps private) **plus up to a cap of cloud handles**. `z`-tag **order is not load-bearing** — a `#z` filter matches any value regardless of position; listing highest-consensus-first is informational only, and **consumers MUST NOT depend on order**.
- **Re-stamping on rotation: lazy author re-emit.** When the cloud has rotated far enough that an author cares, they republish the item at its same `d`-address (kind 39999). Accepted lossiness: foreign-authored items cannot be re-stamped (only the author can re-sign); an inactive author's items fade from discoverability as the cloud rotates away; kind-9999 (non-addressable) items cannot be re-stamped at all — a reason to prefer 39999 for stamped items.
- **Containment only.** This cloud and its stamps are for **containment items** (an item joining a concept's list). A **membership assertion** (asserting a pubkey belongs to a community) keeps its single shared applied-concept handle — the "tag against it" design ([Communities](./communities.md)) — a separate mechanism; the two do not overlap.

Design provenance: worksheet [W11](../worksheet.md#w11--cloud-formation--multi-z-stamping-rules) (now graduated) and `community-reference` ADR 0033.
