...
chown brainstorm:brainstorm ${BRAINSTORM_LOG_DIR}/batchTransfer.log
...
bash "$SCRIPT_DIR/strfryToKind3Events.sh" "$1" "$2"
bash "$SCRIPT_DIR/kind3EventsToFollows.sh"

# add FOLLOWS relationships from followsToAddToNeo4j.json
cypher-shell -u "$NEO4J_USER" -p "$NEO4J_PASSWORD" -a "$NEO4J_URI" -f "$SCRIPT_DIR/apocCypherCommand1_follows" > /dev/null

# update NostrUser kind3EventId and kind3CreatedAt properties by iterating through allKind3EventsStripped.json
cypher-shell -u "$NEO4J_USER" -p "$NEO4J_PASSWORD" -a "$NEO4J_URI" -f "$SCRIPT_DIR/apocCypherCommand2_follows" > /dev/null

# clean up follows

rm /var/lib/neo4j/import/followsToAddToNeo4j.json
rm /var/lib/neo4j/import/allKind3EventsStripped.json
...
# Execute the scripts with full paths
bash "$SCRIPT_DIR/strfryToKind10000Events.sh" "$1" "$2"
bash "$SCRIPT_DIR/kind10000EventsToMutes.sh"

# add MUTES relationships from mutesToAddToNeo4j.json
cypher-shell -u "$NEO4J_USER" -p "$NEO4J_PASSWORD" -a "$NEO4J_URI" -f "$SCRIPT_DIR/apocCypherCommand1_mutes" > /dev/null

# update NostrUser kind10000EventId and kind10000CreatedAt properties by iterating through allKind10000EventsStripped.json
cypher-shell -u "$NEO4J_USER" -p "$NEO4J_PASSWORD" -a "$NEO4J_URI" -f "$SCRIPT_DIR/apocCypherCommand2_mutes" > /dev/null

# clean up mutes

rm /var/lib/neo4j/import/mutesToAddToNeo4j.json
rm /var/lib/neo4j/import/allKind10000EventsStripped.json
...
# Execute the scripts with full paths
bash "$SCRIPT_DIR/strfryToKind1984Events.sh" "$1" "$2"
bash "$SCRIPT_DIR/kind1984EventsToReports.sh"

# add REPORTS relationships from reportsToAddToNeo4j.json
cypher-shell -u "$NEO4J_USER" -p "$NEO4J_PASSWORD" -a "$NEO4J_URI" -f "$SCRIPT_DIR/apocCypherCommand1_reports" > /dev/null

# update NostrUser kind1984EventId and kind1984CreatedAt properties by iterating through allKind1984EventsStripped.json
cypher-shell -u "$NEO4J_USER" -p "$NEO4J_PASSWORD" -a "$NEO4J_URI" -f "$SCRIPT_DIR/apocCypherCommand2_reports" > /dev/null

# clean up reports

rm /var/lib/neo4j/import/reportsToAddToNeo4j.json
rm /var/lib/neo4j/import/allKind1984EventsStripped.json
