...
async function getAllPubkeys() {
  try {
    ...
    const countResult = execSync(`cypher-shell -a "${config.neo4jUri}" -u "${config.neo4jUser}" -p "${config.neo4jPassword}" "${countQuery}" --format plain`).toString();
...
async function processEventKind(pubkeysFile, eventKind, relationshipType) {
  try {
    ...
    execSync(`cypher-shell -a "${config.neo4jUri}" -u "${config.neo4jUser}" -p "${config.neo4jPassword}" --file "${queryFile}" --format plain > "${neo4jIdsFile}"`);
    ...
    for (let i = 0; i < pubkeyList.length; i += strfryBatchSize) {
      ...
      for (const pubkey of batchPubkeys) {
        try {
          ...
          const eventJson = execSync(`strfry scan "{ \\"kinds\\": [${eventKind}], \\"authors\\": [\\"${pubkey}\\"]}" | head -n 1`).toString().trim();
