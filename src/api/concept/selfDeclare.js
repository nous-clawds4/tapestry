...
async function handleConceptSelfDeclare(req, res) {
  try {
    ...
    // Append-only: every existing tag — including other b-tags — survives.
    const newTags = [...(header.tags || []), ['b', selfCoord, 'pointer']];
    await loadTAKey();
    const signed = signAndFinalize({ kind, content: header.content || '', tags: newTags });
