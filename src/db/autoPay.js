...
const deletePaymentStmt = db.prepare(`DELETE FROM auto_payments WHERE claim_event_id = ?`);
...
function spendCapableTotalSince(since, issuerPubkey = null) {
  if (issuerPubkey) {
    return Number(sumSpendCapableByIssuerStmt.get({ since, issuer: issuerPubkey })?.total || 0);
  }
