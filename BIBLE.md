...
**On the person's request an assistant's key also signs its offered identification tagging of them** (today "My Tapestry Owner", against the definition Nous' Tapestry Assistant published; "My Human" is listed but parked, and the route refuses it — identification-tags-authorship ADR 0001), through `POST /api/assistant/identification-tags/publish` — another narrow, session-bound route in the shape of `publish-profile`: the definition is looked up first at its author's address, this instance's relay is written first, and each relay is reported (assistant-identification-tags ADR 0003). Nothing publishes them when an assistant is created; the person presses the button on `/assistant/identification-tags`. The generic signer stays owner-or-admin (OPEN.md row 269) and TA-only, and the other assistant-key signers — trusted lists, curated DList headers and updates, normalization, NIP-85 — are unchanged.
