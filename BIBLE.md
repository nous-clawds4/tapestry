...
**An assistant's key signs two more things on the person's request, and nothing else outside the generic signer's owner-only path:** its two identification taggings of the person ("My Tapestry Owner", "My Human"), through `POST /api/assistant/identification-tags/publish` — session-bound, the canonical tag definitions looked up first, this instance's relay first, each relay reported (assistant-identification-tags ADR 0003). Nothing publishes them when an assistant is created; the person presses the button on `/assistant/identification-tags`.
