...
async function globalSetup(config) {
  ...
  try {
    ...
    // Playwright's FullConfig does not expose a top-level `use` — the shared
    // settings are flattened into each project — so read the resolved value
    // from projects[0] before falling back to the env var / default.
    // (docs/PROFILE_FOLLOWERS_HANDOFF_2026-06-06.md Problem 5)
    const baseURL =
      config?.use?.baseURL ||
