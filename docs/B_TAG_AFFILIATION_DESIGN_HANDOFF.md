...
| 2 | Items carry dual `z` tags (personal header + community header) | **Rejected, rerouted** (D1) — recreates the W1 failure class; inert under existing consumers; redundant with read-side resolution. The underlying need (assert an item into a community's list) routes to the deferred election surface / `dlist-tag` (W10) |
...
*The handleSet recipe:* resolve the community handle `H`; one REQ `{"kinds":[39998], "#b":["H"]}` (a **one-value filter at any community size** — depth-1 makes the result the complete direct-affiliation set; scale pressure lives in the response, never the filter); validate headers (both `b` types count for discovery); trust-gate the *authors* (the whitelist applies to one header per member, not per item); `handleSet = {"39998:"+pubkey+":"+d}`. Maintain by subscription on the same filter + periodic full re-derive (reconciliation backstop). Shortcut: for deterministic-`d` concepts the handleSet is a pure function of the whitelist (`pk → "39998:"+pk+":"+slug`), making the `#b` query a pure *discovery* channel — surfacing unknown affiliates as candidate signal for the W9 roster computation. This promotes slug determinism (O9) to a load-bearing property.

The unbounded transitive walk is eliminated by rule, derived from the founding tenet rather than performance: **affiliation-for-aggregation is depth-1 — direct designation only.** If Bob's header `b`-points at my header and mine at the community's, Bob designated *me*, not the community; counting him in the community aggregate would affiliate him through someone else's act, violating "Centralization is always a hard opt-in, expressed in *your own* declaration." So: community affiliation requires a direct `b` to the community header; the `#b` filter returns the complete aggregation set in one query; transitive closure exists only for *definition resolution* (inherit chains, low-frequency, depth-guarded). Delegation composes via multi-`b` (already allowed): a personal header may carry `["b", <TA-header>, "inherit"]` (maintenance delegation) *and* `["b", <community-header>, "reference"]` (direct affiliation) — orthogonal; only inherit-typed relative order is load-bearing. Zero-derived-state consumers (thin clients on vanilla relays) are served by curator-side projection (Method 2), which recreates the one-filter-no-whitelist pattern with trust-gating moved into the curator's signature. Tapestry instances themselves tabulate against local Neo4j (edges materialized), not relays. *Depth-1 is settled-in-scoping but constrains P1 — confirm at the P1 gate.*
...
**H3 — W1 recurrence via item-level pointers.** Any write-time convention that puts a chosen curator's handle into user-signed item events reproduces the `LEGACY_Z_TAG_PUBKEY` incident class at scale. This killed dual-`z` (D1) and constrains any future election-surface design: the acceptance signal should be curator-side or assertion-layer (taggings), not baked into the member's items.
...
- **O10 — Seeding scope:** manifest concepts only, or runtime-created concepts too (the latter needs a new blessing path).
...
|---|---|---|
| **P1 — `b` type registry + type-gating** | Amending ADR (supersedes ADR 0027's `default "inherit"` text and its restatements in `inherit-from.md` / BIBLE §25/§21, plus ADR 0028's ungated resolution/closure text) | `protocols/drafts/inherit-from.md` (type registry, fail-safe default, type-gated resolution ×2 sites, aggregation scoping, query-shape note); BIBLE §6/§21/§25/§26; `communities.md` (explicit `type "inherit"` wording); worksheet W1/W2/W5/W6 |
| **P2 — `communityReference` v2: seed-not-stub** | ADR | BIBLE §22 (seed semantics, stub retirement, never-clobber rule, coverage widening, the seeds-tags principle); firmware manifest schema notes; ADR 0008 follow-up flag (superset-link promotion) |
