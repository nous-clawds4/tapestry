...
- **Aggregation (the W1 candidate signal) counts inherit-typed edges only.** A bookmark to a definition you disagree with must not count as agreement. v1: reference edges weigh **zero**; graded weighting deferred to the future registry ADR (which ADR 0027 already declares separate and unratified).
